...
# config dasar (samakan dengan train_cat_issue.py)
...
def create_test_loader(device: torch.device):
    if not TEST_DIR.exists():
        raise FileNotFoundError(
            f"Folder test untuk cat issue tidak ditemukan: {TEST_DIR}\n"
            "Pastikan struktur: data_cat_issue/test/<kelas_penyakit>"
        )
    ...
    # optional: cek konsistensi dengan labels_cat_issue.json
    if LABELS_PATH.exists():
        ...
        if set(idx_to_class_train.values()) != set(idx_to_class_test.values()):
            print("⚠️  Peringatan: kelas di test/ tidak sama dengan labels_cat_issue.json")
...
def create_model(num_classes: int, device: torch.device) -> nn.Module:
    # arsitektur harus sama dengan train_cat_issue.py
    ...
    # unfreeze layer4 seperti di training
...
def main():
    ...
    print(f"Jumlah kelas cat issue di test: {num_classes}")

    if not MODEL_PATH.exists():
        raise FileNotFoundError(
            f"File model cat issue tidak ditemukan: {MODEL_PATH}\n"
            "Pastikan sudah menjalankan train_cat_issue.py terlebih dahulu."
        )

    model = create_model(num_classes, device)
    state = torch.load(MODEL_PATH, map_location=device)
    model.load_state_dict(state)
    print(f"✅ Cat-issue model weights loaded from {MODEL_PATH}")
